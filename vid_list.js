...
function encWbi(params) {
  const img_url = window.localStorage.wbi_img_url;
  const img_key = img_url.substring(img_url.lastIndexOf('/') + 1, img_url.length).split('.')[0];
  const sub_url = window.localStorage.wbi_sub_url;
...
async function loadVidList(mid = location.href.split('/')[3]) {
  let pn = 1;
  let ret = [];
  while (true) {
    console.log('输出第', pn, '页');
    ...
    ret = ret.concat(data.data.list.vlist);
    const totalPage = Math.floor(data.data.page.count / data.data.page.ps) + 1;
    if (pn === totalPage) {
      break;
    }
    pn++;
  }
  const exportData = ret.map(e => {
    return `https://www.bilibili.com/video/${e.bvid}\r\n${e.title}\r\n${e.description}\r\n`;
  });
